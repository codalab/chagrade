...
class ProfilesSmokeTests(TestCase):
    ...
    def test_edit_klass_returns_200(self):
        #  creating the instructor and klass objects for the test
        instructor = Instructor.objects.create()
        klass = Klass.objects.create(instructor=instructor, course_number="1")

        # Trying not logged in to edit klass
        resp = self.client.get(reverse('klasses:edit_klass', kwargs={'klass_pk': klass.pk}))
        assert resp.status_code == 302
        assert resp.url.startswith(reverse('profiles:login'))
    ...
    def test_klass_details_returns_200(self):
        ...
        resp = self.client.get(reverse('klasses:klass_details', kwargs={'klass_pk': klass.pk}))
        assert resp.status_code == 302
        assert resp.url.startswith(reverse('profiles:login'))
    ...
    def test_klass_enrollment_returns_200(self):
        ...
        resp = self.client.get(reverse('klasses:klass_enrollment', kwargs={'klass_pk': klass.pk}))
        assert resp.status_code == 302
        assert resp.url.startswith(reverse('profiles:login'))
    ...
    def test_klass_homework_view_returns_200(self):
        ...
        resp = self.client.get(reverse('klasses:klass_homework', kwargs={'klass_pk': klass.pk}))
        assert resp.status_code == 302
        assert resp.url.startswith(reverse('profiles:login'))
    ...
    def test_klass_grading_homework_view_returns_200(self):
        ...
        resp = self.client.get(reverse('klasses:klass_grading', kwargs={'klass_pk': klass.pk}))
        assert resp.status_code == 302
        assert resp.url.startswith(reverse('profiles:login'))
    ...
    def test_klass_activate_view_returns_200(self):
        ...
        # Trying not logged in to view klass homework
        resp = self.client.get(reverse('klasses:klass_activate', kwargs={'klass_pk': klass.pk}))
        assert resp.status_code == 302
        assert resp.url.startswith(reverse('profiles:login'))
    ...
    def test_homework_define_homework_returns_200(self):
        ...
        # Trying not logged in to edit klass
        resp = self.client.get(reverse('homework:define_homework', kwargs={'klass_pk': klass.pk}))
        assert resp.status_code == 302
        assert resp.url.startswith(reverse('profiles:login'))
    ...
    def test_homework_grade_homework_view_returns_200(self):
        ...
        fakestudent = StudentMembership.objects.create(user=faker, klass=klass, student_id=1)
        submission = Submission.objects.create(klass=klass, creator=fakestudent)

        # Trying not logged in to edit klass
        resp = self.client.get(reverse('homework:grade_homework',
                                       kwargs={'klass_pk': klass.pk, 'submission_pk': submission.pk}))
        assert resp.status_code == 302
        assert resp.url.startswith(reverse('profiles:login'))
    ...
    def test_homework_submit_homework_view_returns_200(self):
        ...
        # Trying not logged in to edit klass
        resp = self.client.get(reverse('homework:submit_homework', kwargs={'klass_pk': klass.pk}))
        assert resp.status_code == 302
        assert resp.url.startswith(reverse('profiles:login'))
